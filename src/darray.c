...
darray_fmread(fmcmb_t stream)
{
	...
	if (!fm_read_int32(stream, &n) || DA_SIGNATURE != (uint32_t)n) {
		goto exit_file_read;
	}
	if ((d = malloc(sizeof(*d))) == NULL) {
		goto exit_file_read;
	...
	if (!fm_read_int32(stream, &d->num_cells)) {
		goto exit_da_created;
	}
	...
	for (n = 1; n < d->num_cells; n++) {
		if (!fm_read_int32 (stream, &d->cells[n].base) ||
		    !fm_read_int32 (stream, &d->cells[n].check)) {
...
darray_fmwrite(const darray_t d, fmcmb_t stream)
{
	...
	for (i = 0; i < d->num_cells; i++) {
		if (!fm_write_int32 (stream, d->cells[i].base) ||
		    !fm_write_int32 (stream, d->cells[i].check)) {
			return -1;
		}
	}
	return 0;
